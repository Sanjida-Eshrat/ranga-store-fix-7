...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    `<div class="single-product">
        ...
        <h2>Price: $ <span class="txt-color">${product.price}</span></h2>
        <p><b>Rating: <span class="txt-color">${product.rating.rate}</span>, Total Rating: <span class="txt-color">${product.rating.count}</span></b></p>
        <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-pink text-white"><i class="fas fa-shopping-cart"></i> Add to cart</button>
        <button id="details-btn" class="btn btn-color text-white">Details</button>
      </div>
      `;
...
const addToCart = (id, price) => {
  ...
  // calculate total added product price
  updatePrice("price", price);
  // calculate tax and delivery charge
  updateTaxAndCharge();
  document.getElementById("total-Products").innerText = countProduct;
  // calling update total 
  updateTotal();
};
// Get cart values to calculate total
...
// calling loadProducts to show all products
loadProducts();